...
describe('SERVICE UPDATE', () =>{
  ...
  describe('SERVICE UPDATE POSITIVE', () =>{
    before(async() =>{
      getService = await serviceHelper.getServiceById(service.body.payload)
  ...
  describe('SERVICE UPDATE WITHOUT REQUIRED NAME FIELD', () =>{
    before(async() =>{
     updateService = await serviceHelper.updateServiceById(service.body.payload, {...serviceHelper.serviceData(vendorId), name: ''})
    })
  ...
  describe('SERVICE UPDATE WITHOUT REQUIRED CLIENT PRICE FIELD', () =>{
    before(async() =>{
      updateService = await serviceHelper.updateServiceById(service.body.payload, {...serviceHelper.serviceData(vendorId), clientPrice: ''})
    })
    it('verify status code for update request is 400', () =>{
      expect(updateService.statusCode).to.eq(400);
    })
    ...
    it('can`t update service without required clientPrice field', () =>{
      expect(getService.body.payload.clientPrice).to.not.eq('');
    })
  })
  describe('SERVICE UPDATE WITHOUT REQUIRED VENDOR PRICE FIELD', () =>{
    before(async() =>{
      updateService = await serviceHelper.updateServiceById(service.body.payload, {...serviceHelper.serviceData(vendorId), vendorPrice: ''})
    })
    it('verify status code for update request is 400', () =>{
      expect(updateService.statusCode).to.eq(400);
    })
